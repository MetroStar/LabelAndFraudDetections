# ML_FraudDetections
Used for the TTB efforts to detect fraudulent labels and formulas

# Instructions to configure LabelValidation.ipynb
***
# When in a local laptop/desktop, need to pip3 install -r requirements.txt

# From the SageMaker Homepage select "Open Jupyter"
![test info](images/SageMaker_Homepage.png)

# From the Jupyter Notebook "Upload" LabelValidation.ipynb, and labelImages.zip
![test info](images/Upload_Image.png)

# Unzip labelImages.zip in a terminal
# Open a terminal
![test info](images/Get_To_Terminal.png)
# Go to the terminal, cd to SageMaker, mkdir TestData, and in TestData unzip labelImages.zip
![test info](images/Terminal_SageMaker.png)
# Click to open LabelValidation.ipynb
# When in  Jupyter Notebook, need to use the conda_tensorflow_p36 kernel 
![test info](images/Change_Kernel.png)

# Instructions to configure FormulaFraudDetection.ipynb
***
# When in a local laptop/desktop, need to pip3 install -r requirements.txt

# From the SageMaker Homepage select "Open Jupyter"
![test info](images/SageMaker_Homepage.png)

# From the Jupyter Notebook "Upload" FormulaFraudDetection.ipynb, and formula_fraud.zip
![test info](images/Upload_Image.png)

# Unzip formula_fraud.zip in a terminal
# Open a terminal
![test info](images/Get_To_Terminal.png)
# Go to the terminal, cd to SageMaker, mkdir TestData, and in TestData unzip formula_fraud.zip
![test info](images/Terminal_SageMaker.png)
# Click to open FormulaFraudDetection.ipynb
# When in  Jupyter Notebook, need to use the conda_python3 kernel 
![test info](images/Change_Kernel.png)
